...
function Invoke-GitChecked {
    ...
    $lines = @(& $gitExe -C $Repo @Arguments)
    if ($LASTEXITCODE -ne 0) { throw "git $($Arguments -join ' ') failed. Existing work was left in place." }
    return $lines
}
function GitText {
    param([Parameter(Mandatory)][string[]]$Arguments)
    return ((Invoke-GitChecked -Arguments $Arguments) -join "`n").Trim()
}
...
if ($head -eq $base) {
    if ($dirty.Length -eq 0) {
        Invoke-GitChecked -Arguments @('apply', '--check', $patch) | Out-Host
        $needsApply = $true
    }
    else {
        ...
        $changed = @(Invoke-GitChecked -Arguments @('diff', '--name-only', 'HEAD'))
...
if ($head -eq $base) {
    if ($needsApply) { Invoke-GitChecked -Arguments @('apply', $patch) | Out-Host }
    Invoke-GitChecked -Arguments @('add', '--', $file) | Out-Host
    Invoke-GitChecked -Arguments @('diff', '--cached', '--check') | Out-Host
    ...
    Invoke-GitChecked -Arguments @('diff', '--cached', '--stat') | Out-Host
    Invoke-GitChecked -Arguments @('commit', '-S', '-m', 'fix: exclude RAW decoders from content fallback', '-m', 'Remove unused fallback exception Data as requested in review 5199828960.') | Out-Host
    $head = GitText -Arguments @('rev-parse', 'HEAD')
}
Invoke-GitChecked -Arguments @('verify-commit', 'HEAD') | Out-Host
...
Invoke-GitChecked -Arguments @('push', 'origin', "HEAD:$target") | Out-Host
